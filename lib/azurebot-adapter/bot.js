...
async function getReply(conversationId, conversationToken, activityId, remainingAttempts = 4) {
    try {
        ...
        if (lastActivity.replyToId == activityId) {
            ...
            throw new Error("Azure Bot: didn't reply to last message");
        }
    } catch (error) {
        if (remainingAttempts === 0) throw new Error(`Azure Bot: get reply failed too many times. Last error: ${error.message}`);
        await waitFor(500);
